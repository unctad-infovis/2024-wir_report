...
function App() {
  ...
  useEffect(() => {
    const data_file = (window.location.href.includes('unctad.org')) ? 'https://storage.unctad.org/2024-wir_report/assets/data/2024-wir_report_figure1.json' : './assets/data/2024-wir_report_figure1.json';
    try {
      fetch(data_file)
        .then((response) => {
          if (!response.ok) {
  ...
  const createChart = useCallback(() => {
    chart = Highcharts.chart('highchart-container', {
      ...
      xAxis: {
        accessibility: {
          description: 'Year from 1990 to 2023'
        },
        allowDecimals: false,
        crosshair: {
